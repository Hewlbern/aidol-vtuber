...
def sync_user_config(config_dir="config"):
    """Sync user config with default config"""
    config_path = Path(config_dir) / "templates"
    user_config = Path("conf.yaml") 
    default_config = config_path / "conf.default.yaml"
    zh_config = config_path / "conf.ZH.default.yaml"

    if not user_config.exists():
        # Copy default config based on language
        source = zh_config if lang == "zh" else default_config
        shutil.copy2(source, user_config)
...
def perform_upgrade(custom_logger=None):
    ...
    # Update config
    sync_user_config(config_dir="config")  # merge user config
