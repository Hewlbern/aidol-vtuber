...
export function NewAgentWindow({ isOpen, onClose, onSave }: NewAgentWindowProps) {
  ...
  const [formData, setFormData] = useState<Agent>(defaultFormData);

  // Reset form when window opens/closes
  useEffect(() => {
    if (isOpen) {
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h2 className="font-medium text-black">Activate Agent</h2>
                ...
                <div>
                  ...
                  >
                    ...
                    {Object.values(agents).map((agent) => (
                      <option key={agent.agentId} value={agent.name}>
        ...
        {/* Footer */}
        <div className="border-t p-4 flex justify-end items-center bg-white">
          <button
            onClick={handleSave}
            disabled={isSaving}
          ...
          >
            {isSaving ? 'Activating...' : 'Activate Agent'}
          </button>
        </div>
      </div>
    </>
